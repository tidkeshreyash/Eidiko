...
//    }
    ...
    @GetMapping("/userAadhaarData/{id}")
    public ResponseEntity<String> getAadhaarCardData(@PathVariable Long id) {
        try {
            // Fetch Aadhaar card data using the service method
            String aadhaarData = userService.getAadhaarCardData(id);

            // If Aadhaar data was successfully extracted
            if (aadhaarData != null) {
            ...
            return ResponseEntity.status(500).body("Error while processing Aadhaar data: " + e.getMessage());
        }
    }
